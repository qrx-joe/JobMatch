...
xlsx_files = [
    ...
    if not os.path.basename(f).startswith("~$") and "标黄岗位统计信息" not in f
]
...
for sheet_name in wb.sheetnames:
    ...
    # 合并表头
    headers = []
    for _i, (h1, h2) in enumerate(zip(header_row1, header_row2, strict=False)):
...
# 统计摘要
total_recruit = sum(
    int(job["招募人数"]) if str(job["招募人数"]).isdigit() else 0 for job in yellow_jobs
)
