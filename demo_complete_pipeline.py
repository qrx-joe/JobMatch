...
from historical_analyzer import enrich_jobs_with_historical_data
from job_matcher_v3 import Job, JobMatcherV3, MatchLevel
from report_generator import MatchReport, ReportGenerator
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    job = Job(
                        ...
                        recruit_count=_parse_int(
                            _get_field(row, ["招募人数", "招录人数", "人数", "计划"]), 1
                        ),
...
def analyze_competition(jobs: list[Job]) -> dict:
    ...
    # 最容易的岗位
    easiest = sorted(
        [j for j in jobs if j.competition_ratio > 0], key=lambda x: x.competition_ratio
    )[:5]
...
def display_results(jobs: list[Job], show_top: int = 10):
    ...
    for i, job in enumerate(jobs[:show_top], 1):
        ...
        # 报名数据
        stats = (
            f"报名{job.applicants}/初审{job.approved}/缴费{job.paid}"
            if job.paid > 0
            else "暂无报名数据"
        )
...
def export_results(
    jobs: list[Job], profile: dict, output_dir: str = "reports", config: dict = None
):
    """导出结果（支持历史数据）"""
    print("\n[导出报告]")
    ...
    report = MatchReport(
        ...
        summary={
            ...
            "avg_predicted_score": (
                sum(j.predicted_score for j in jobs if j.predicted_score > 0)
                / sum(1 for j in jobs if j.predicted_score > 0)
            )
            if sum(1 for j in jobs if j.predicted_score > 0) > 0
            else 0,
        },
    )
    ...
    for i, job in enumerate(jobs[:show_top], 1):
        ...
        if job.historical_stats:
            ...
            if job.predicted_score > 0:
                print(
                    f"    [预测] 今年预测上岸分: {job.predicted_score:.1f}分 | 趋势: {job.difficulty_trend}"
                )
