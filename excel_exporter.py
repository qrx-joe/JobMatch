            ...
            for c_idx, value in enumerate(row, 1):
                ...
                if r_idx == 1:
                    ...
                    if match_level == MatchLevel.PERFECT.value:
                        cell.fill = PatternFill(start_color=self.COLORS['perfect'],
                                               end_color=self.COLORS['perfect'],
                                               fill_type='solid')
                    elif match_level == MatchLevel.PARTIAL.value:
                        cell.fill = PatternFill(start_color=self.COLORS['partial'],
                                               end_color=self.COLORS['partial'],
                                               fill_type='solid')
                    else:
                        cell.fill = PatternFill(start_color=self.COLORS['mismatch'],
...
if __name__ == "__main__":
    ...
    test_jobs = [
        Job(sheet_name='太原市', unit='测试单位1', job_type='管理1',
            major='经济学类', education='本科及以上',
            match_level=MatchLevel.PERFECT, match_score=100,
            paid=50, recruit_count=1, competition_ratio=50.0),
        Job(sheet_name='吕梁市', unit='测试单位2', job_type='专技1',
            major='不限', education='本科及以上',
            match_level=MatchLevel.PARTIAL, match_score=80,
            paid=30, recruit_count=1, competition_ratio=30.0),
        Job(sheet_name='大同市', unit='测试单位3', job_type='管理2',
            major='法学', education='研究生',
            match_level=MatchLevel.MISMATCH, match_score=0,
            paid=100, recruit_count=1, competition_ratio=100.0),
    ]
