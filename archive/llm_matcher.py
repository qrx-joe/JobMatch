...
class LLMMajorMatcher:
    ...
    def match(self, job_major: str) -> tuple[bool, str, str]:
        ...
        if self.use_llm:
            ...
            # 缓存结果
            self.cache[cache_key] = {"match": llm_result[0], "reason": llm_result[1]}
            self._save_cache()
    ...
    def _extract_keywords(self, text: str) -> set:
        ...
        for part in text.split():
            ...
            if len(part) >= 2:
                # 移除"类"、"学"等后缀
                if part.endswith("类") or part.endswith("学") and len(part) > 3:
                    words.append(part[:-1])
                else:
                    words.append(part)
        return set(words)
    ...
    def _llm_match(self, job_major: str) -> tuple[bool, str]:
        ...
        job_cores = [
            self._get_core_major(m)
            for m in job_major.replace("、", ",").replace("，", ",").split(",")
        ]
...
def test_matcher():
    ...
    test_cases = [
        ...
        (
            "经济学",
            "本科及以上，学士及以上，经济学类（0201）、财政学类（0202）、金融学类（0203）",
            True,
        ),
    ]
    ...
    for user_major, job_major, expected in test_cases:
        ...
        print(
            f"   结果: {'匹配' if match else '不匹配'} (期望: {'匹配' if expected else '不匹配'})"
        )
        print(f"   说明: {reason}")
